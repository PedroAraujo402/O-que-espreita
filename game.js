// ─────────────────────────────────────────────
// MONSTER ASCII ART — progressive reveal
// ─────────────────────────────────────────────
// Frame 0-10: monstro se forma gradualmente
...
// Versão final grande para o game over
const MONSTER_FINAL = `
...
// ─────────────────────────────────────────────
// MONSTER OVERLAY — progressive reveal
// ─────────────────────────────────────────────
function updateMonsterOverlay() {
  ...
  // Mapeia proximidade 30-100 para frames 0-10
  const frameIndex = Math.min(10, Math.floor((proximity - 30) / 7));
  const monsterText = MONSTER_FRAMES[frameIndex] || '';

  // Opacidade cresce gradualmente
  const opacity = Math.min((proximity - 30) / 70, 0.9);
  layer.style.opacity = String(opacity);

  // Repete o monstro para preencher a área visível
  const linesNeeded = Math.ceil(document.getElementById('output').clientHeight / 22) + 3;
  const monsterLines = monsterText.split('\n');
  let fullText = '';
  for (let i = 0; i < linesNeeded; i++) {
    fullText += monsterLines[i % monsterLines.length] + '\n';
  }
  layer.textContent = fullText;

  // Cor fica mais intensa conforme proximidade
  if (proximity > 80) {
    layer.style.color = '#8b000066';
    layer.style.textShadow = '0 0 4px #8b000044';
  } else if (proximity > 60) {
    layer.style.color = '#8b000044';
    layer.style.textShadow = 'none';
  } else if (proximity > 40) {
    layer.style.color = '#8b000022';
    layer.style.textShadow = 'none';
  } else {
    layer.style.color = '#8b000011';
    layer.style.textShadow = 'none';
  }
}
...
function triggerGameOver(finalText) {
  ...
  // Escurece tudo
  output.style.transition = 'background 1.5s';
  output.style.background = '#000';

  // Esconde o overlay do monstro e mostra a versão final gigante
  document.getElementById('monster-layer').style.opacity = '0';

  setTimeout(() => {
    // Corrompe o texto final
    const corruptedFinal = corruptText(MONSTER_FINAL, 100);
    ...
    gameDiv.style.textShadow = '0 0 10px #ff000066, 0 0 30px #8b000044';
    gameDiv.style.whiteSpace = 'pre';
    gameDiv.style.fontSize = '0.75rem';
    gameDiv.style.lineHeight = '1.4';
    gameDiv.innerHTML = corruptedFinal + '\n\n\n<span style="color:#ff0000;font-size:1.2rem;text-shadow:0 0 20px #ff0000;">[ FIM ]</span>';
    entry.appendChild(gameDiv);
